...
\begin{itemize}
	Possiamo notare che il logo \`e\ in alto a destra, ben visibile e con un nome che lo identifica. Sotto questo aspetto l'utente capisce subito che si tratta di  un sito che identifica una scuola. (da rivedere..)
\end{itemize}
\subsection{Where} Dove ci troviamo?
\begin{itemize}
	Si pu\`o\ subito notare che vi sono pochi strumenti che offrono all'utente la possibilit\`a\ di orientarsi:
	l'assenza di qualsiasi tipo di breadcrumb(attributo,path), non permette all'utente
	di sapere dove si trova. Molto spesso 
\end{itemize}
